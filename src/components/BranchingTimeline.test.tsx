...
describe("BranchingTimeline", () => {
  it("renders only selected identity timeline items and opens the clicked item", () => {
    ...
    expect(screen.getByRole("button", { name: "Interior A" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Pair A" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Pair B" })).not.toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Pair A" }));
